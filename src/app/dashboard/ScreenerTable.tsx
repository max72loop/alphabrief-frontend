      ...
      {sub && (
        <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          <div className="bg-[#0d0d1a] border border-white/15 rounded-lg p-3 shadow-2xl w-48 text-left">
            <div className="text-[0.6rem] uppercase tracking-widest text-[#4A6355] mb-2.5">Détail du score</div>
            {([
              ...
              return (
                <div key={label} className="mb-2 last:mb-0">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-[#C6C0A9]">{label} <span className="text-[#4A6355]">{weight}</span></span>
                    <span className={`text-xs font-bold tabular-nums ${tierColor[t]}`}>{val}</span>
                  </div>
                  <div className="h-1 bg-[#13201A] rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${val >= 65 ? 'bg-[#7EE5A3]' : val >= 40 ? 'bg-[#E5A04E]' : 'bg-[#D85F66]'}`}
                      style={{ width: `${val}%` }}
                    />
                  </div>
                </div>
  ...
  function SortTh({ colKey, label, className = '' }: { colKey: SortKey; label: string; className?: string }) {
    ...
    return (
      <th
        ...
        className={`px-4 py-3 text-xs font-medium text-[#6D7A72] uppercase tracking-wider cursor-pointer hover:text-[#7EE5A3] select-none whitespace-nowrap ${className}`}
      >
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {([
          { key: 'all',  label: `Tous (${rows.length})`,       cls: signal === 'all'  ? 'bg-[#7EE5A3]/15 text-[#7EE5A3] border-[#7EE5A3]/30' : '' },
          { key: 'high', label: `Score fort (${counts.high})`, cls: signal === 'high' ? 'bg-[#7EE5A3]/15 text-[#7EE5A3] border-emerald-500/30' : '' },
          { key: 'mid',  label: `Score moyen (${counts.mid})`, cls: signal === 'mid'  ? 'bg-[#E5A04E]/15 text-[#E5A04E] border-amber-500/30' : '' },
          { key: 'low',  label: `Score faible (${counts.low})`,cls: signal === 'low'  ? 'bg-[#D85F66]/15 text-[#D85F66] border-rose-500/30' : '' },
        ] as const).map(chip => (
          <button
            key={chip.key}
            onClick={() => setSignal(chip.key)}
            className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all
              ${chip.cls || 'border-[#1A2520] bg-[#13201A] text-[#6D7A72] hover:text-[#F0EBDB] hover:bg-[#1A2520]'}`}
          >
            {chip.label}
          </button>
        ))}
      ...
      {/* Filters */}
      <div className="rounded-xl border border-[#1A2520] bg-[#0E1511] px-4 py-3 mb-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[0.65rem] text-[#4A6355] uppercase tracking-wider">Secteur</label>
            <select
              value={sector}
              onChange={e => setSector(e.target.value)}
              className="px-2.5 py-1.5 rounded-lg bg-[#13201A] border border-[#1A2520] text-sm text-[#F0EBDB] focus:border-[#7EE5A3]/50 outline-none cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[0.65rem] text-[#4A6355] uppercase tracking-wider">Score min</label>
            <input type="number" min="0" max="100" placeholder="0" value={minScore}
              onChange={e => setMinScore(e.target.value)}
              className="px-2.5 py-1.5 rounded-lg bg-[#13201A] border border-[#1A2520] text-sm text-[#F0EBDB] focus:border-[#7EE5A3]/50 outline-none w-20"
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[0.65rem] text-[#4A6355] uppercase tracking-wider">RSI max</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[0.65rem] text-[#4A6355] uppercase tracking-wider">Mom 3m min (%)</label>
            <input type="number" placeholder="ex : -10" value={minMom}
              onChange={e => setMinMom(e.target.value)}
              className="px-2.5 py-1.5 rounded-lg bg-[#13201A] border border-[#1A2520] text-sm text-[#F0EBDB] focus:border-[#7EE5A3]/50 outline-none w-24"
            />
          </div>
          ...
          <div className="ml-auto flex gap-2 items-end">
            {hasFilters && (
              <button
                ...
                className="px-3 py-1.5 rounded-lg border border-[#1A2520] text-xs text-[#4A6355] hover:border-rose-500/40 hover:text-[#D85F66] transition-colors"
              >
                ✕ Réinitialiser
              </button>
            )}
            ...
            <div className="relative">
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-[#13201A] border border-[#1A2520] text-sm text-[#C6C0A9] hover:text-[#F0EBDB] hover:border-white/20 transition-colors whitespace-nowrap"
              >
                Colonnes ▾
              </button>
              {colOpen && (
                <div className="absolute right-0 top-full mt-1.5 bg-[#13131f] border border-white/10 rounded-lg p-1.5 z-50 min-w-[180px] shadow-2xl">
                  <div className="text-[0.6rem] uppercase tracking-widest text-[#4A6355] px-2 py-1">Afficher / masquer</div>
                  {OPTIONAL_COLS.map(c => (
                    <label key={c.key} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-[#13201A] cursor-pointer text-sm text-[#C6C0A9] hover:text-[#F0EBDB] transition-colors">
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-[#1A2520] px-6 py-12 text-center">
          <p className="text-[#F0EBDB] font-semibold mb-1">Aucun résultat</p>
          <p className="text-[#4A6355] text-sm mb-5">Tes filtres actifs ne correspondent à aucun titre.</p>
          {/* Active filters as removable chips */}
          <div className="flex flex-wrap gap-2 justify-center mb-6">
            {search && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                Recherche : <span className="text-[#F0EBDB] font-medium">{search}</span>
                <button onClick={() => setSearch('')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
            {sector !== 'all' && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                Secteur : <span className="text-[#F0EBDB] font-medium">{sector}</span>
                <button onClick={() => setSector('all')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
            {minScore && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                Score ≥ <span className="text-[#F0EBDB] font-medium">{minScore}</span>
                <button onClick={() => setMinScore('')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
            {maxRsi && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                RSI ≤ <span className="text-[#F0EBDB] font-medium">{maxRsi}</span>
                <button onClick={() => setMaxRsi('')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
            {minMom && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                Mom 3m ≥ <span className="text-[#F0EBDB] font-medium">{minMom}%</span>
                <button onClick={() => setMinMom('')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
            {signal !== 'all' && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs bg-[#13201A] border border-[#1A2520] text-[#C6C0A9]">
                Signal : <span className="text-[#F0EBDB] font-medium">{signal === 'high' ? 'Fort' : signal === 'mid' ? 'Moyen' : 'Faible'}</span>
                <button onClick={() => setSignal('all')} className="text-[#4A6355] hover:text-[#D85F66] transition-colors">×</button>
              </span>
            )}
          </div>
          {/* Quick actions */}
          <div className="flex flex-wrap gap-2 justify-center">
            <button
              ...
              className="px-4 py-2 rounded-lg bg-[#7EE5A3]/10 border border-emerald-500/20 text-[#7EE5A3] text-xs font-semibold hover:bg-[#7EE5A3]/20 transition-colors"
            >
              Voir les scores forts
            </button>
            <button
              onClick={() => { setSearch(''); setSector('all'); setMinScore(''); setMaxRsi(''); setMinMom(''); setSignal('all') }}
              className="px-4 py-2 rounded-lg bg-[#13201A] border border-[#1A2520] text-[#C6C0A9] text-xs font-semibold hover:text-[#F0EBDB] hover:border-white/20 transition-colors"
            >
              Tout réinitialiser
            </button>
          </div>
        ...
        <div className="rounded-xl border border-[#1A2520] overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1A2520] bg-[#0E1511]">
                ...
                {col('score_date')  && <th className="px-4 py-3 text-xs font-medium text-[#6D7A72] uppercase tracking-wider text-center">Mis à jour</th>}
              ...
              {filtered.map(row => {
                ...
                return (
                  <tr
                    ...
                    className="hover:bg-[#7EE5A3]/[0.05] transition-colors cursor-pointer"
                  >
                    <td className="pl-5 pr-4 py-3.5">
                      <div className="font-bold text-[#F0EBDB]">{row.company_name || row.ticker}</div>
                      {row.company_name && (
                        <div className="text-xs text-[#4A6355] mb-1.5">{row.ticker}</div>
                      )}
                      <div className="flex items-center gap-1.5">
                        {([
                          ...
                          <div key={label} title={`${label === 'F' ? 'Fondamentaux' : label === 'T' ? 'Techniques' : 'Momentum'}: ${v}`} className="flex items-center gap-0.5">
                            <span className="text-[0.5rem] text-[#4A6355] font-medium w-2.5">{label}</span>
                            <div className="w-8 h-1 bg-[#13201A] rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${v >= 65 ? 'bg-[#7EE5A3]/60' : v >= 40 ? 'bg-[#E5A04E]/60' : 'bg-[#D85F66]/60'}`}
                                style={{ width: `${v}%` }}
                              />
                            </div>
                          </div>
                    ...
                    {col('rsi') && (
                      <td className={`px-4 py-3.5 text-right tabular-nums text-sm ${rsi != null && rsi > 70 ? 'text-[#D85F66]' : rsi != null && rsi < 30 ? 'text-sky-400' : 'text-[#C6C0A9]'}`}>
                        {fmt(rsi)}
                      </td>
                    )}
                    {col('mom3m') && (
                      <td className={`px-4 py-3.5 text-right tabular-nums text-sm ${mom3m != null && mom3m > 0 ? 'text-[#7EE5A3]' : mom3m != null && mom3m < 0 ? 'text-[#D85F66]' : 'text-[#C6C0A9]'}`}>
                        {mom3m != null ? fmtPct(mom3m) : '—'}
                      </td>
                    )}
                    {col('mom12m') && (
                      <td className={`px-4 py-3.5 text-right tabular-nums text-sm ${mom12m != null && mom12m > 0 ? 'text-[#7EE5A3]' : mom12m != null && mom12m < 0 ? 'text-[#D85F66]' : 'text-[#C6C0A9]'}`}>
                        {mom12m != null ? fmtPct(mom12m) : '—'}
                      </td>
                    )}
                    {col('pe') && (
                      <td className="px-4 py-3.5 text-right tabular-nums text-sm text-[#C6C0A9]">
                    ...
                    {col('score_date') && (() => {
                      ...
                      return (
                        <td className="px-4 py-3.5 text-center">
                          <span className={`text-xs tabular-nums ${stale ? 'text-[#D85F66]' : 'text-[#4A6355]'}`}
      ...
      <p className="text-xs text-[#4A6355] mt-3">
